      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          {testimonials.map((testimonial, index) => (
            <SwiperSlide key={index} className={`swiper-slide ${index === 1 ? 'swiper-slide-center' : ''}`}>
              <img
                src={testimonial.image}
                alt={`Experience ${index + 1}`}
                className={`w-full h-auto object-cover ${index === 1 ? 'swiper-slide-center' : ''}`}
              />
            </SwiperSlide>
          ))}
        </Swiper>
